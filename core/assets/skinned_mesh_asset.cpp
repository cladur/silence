...
assets::SkinnedVertexFormat parse_skinned_mesh_format(const char *f) {
	if (strcmp(f, "PNV32") == 0) {
		...
		return assets::SkinnedVertexFormat::P32N8C8V16;
	} else {
		return assets::SkinnedVertexFormat::Unknown;
	}
}

assets::SkinnedMeshInfo assets::read_skinned_mesh_info(AssetFile *file) {
...
void assets::unpack_mesh(
		SkinnedMeshInfo *info, const char *source_buffer, size_t source_size, char *vertex_buffer, char *index_buffer) {
...
assets::AssetFile assets::pack_mesh(SkinnedMeshInfo *info, char *vertex_data, char *index_data) {
	AssetFile file;
	file.type[0] = 'M';
	file.type[1] = 'E';
	file.type[2] = 'S';
	...
	if (info->vertex_format == SkinnedVertexFormat::P32N8C8V16) {
		metadata["vertex_format"] = "P32N8C8V16";
	} else if (info->vertex_format == SkinnedVertexFormat::PNV32) {
...
assets::SkinnedMeshBounds assets::calculate_bounds(SkinnedVertexPNV32 *vertices, size_t count) {
