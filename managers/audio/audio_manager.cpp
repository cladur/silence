...
void AudioManager::load_startup_banks() {
	FMOD_CHECK(system->loadBankFile((path_to_banks + "Master" + bank_postfix).c_str(), FMOD_STUDIO_LOAD_BANK_NORMAL, &master_bank));
	SPDLOG_INFO("Audio Manager: Loaded bank: {}", (path_to_banks + "Master" + bank_postfix).c_str());
	FMOD_CHECK(system->loadBankFile((path_to_banks + "Master.strings" + bank_postfix).c_str(), FMOD_STUDIO_LOAD_BANK_NORMAL, &strings_bank));
...
void AudioManager::load_bank(const std::string &name) {
	...
	FMOD_CHECK(system->loadBankFile((path_to_banks + name + bank_postfix).c_str(), FMOD_STUDIO_LOAD_BANK_NORMAL, &bank));
