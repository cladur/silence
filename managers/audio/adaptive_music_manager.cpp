...
AutoCVarInt enable_music("music.enabled", "is music enabled", 1, CVarFlags::EditCheckbox);
...
void AdaptiveMusicManager::update(float dt) {
	...
	float drum_intensity = (-(drum_int_calc_val / ((float)enemy_near_count + drum_int_calc_val)) + 1.0f) * 0.8f +
			detection_to_intensity * 0.2f;
	...
	float melodic_intensity = (float)is_sprinting * 0.1f + detection_to_intensity * 0.5f + (float)is_crouching * -0.1f +
			std::clamp((float)enemy_near_count * 0.15f, 0.0f, 1.0f) * 0.55f;
	...
	event_instance->setVolume((float)enable_music.get());
}
