...
public:
	...
	std::map<uint32_t, std::pair<std::array<ParticleData, MAX_PARTICLES_PER_ENTITY>, ParticlePerEntityData>> particles;
	...
	void startup();
	void shutdown();
	void emit(uint32_t entity, ParticleData particle, ParticlePerEntityData entity_data);
