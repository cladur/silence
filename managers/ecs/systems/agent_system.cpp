...
void AgentSystem::update(World &world, float dt) {
	...
	for (const Entity entity : entities) {
		...
		Ray ray{};
		ray.origin = transform.get_global_position() + glm::vec3(0.0f, 1.f, 0.0f);
		ray.ignore_list.emplace_back(entity);
		...
		if (CollisionSystem::ray_cast(world, ray, info)) {
			...
			world.get_parent_scene()->get_render_scene().debug_draw.draw_arrow(info.point, info.point + info.normal * 10.0f);
